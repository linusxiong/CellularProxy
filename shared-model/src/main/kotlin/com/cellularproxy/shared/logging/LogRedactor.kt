...
object LogRedactor {
    private val sensitiveHeaderLineRegex =
        Regex(
            pattern = """(?im)^(\s*(?:Authorization|Proxy-Authorization|Cookie|Set-Cookie)\s*:\s*).*$""",
        )
    private val sensitiveHeaderFieldRegex =
        Regex(
    ...
    private val absoluteUrlQueryRegex =
        Regex(
            pattern = """\b(https?://[^\s?#"'<>),\]}]+)\?[^\s#"'<>),\]}]*(#[^\s"'<>),\]}]*)?""",
        )
    private val relativePathQueryRegex =
        Regex(
