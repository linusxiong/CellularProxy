...
sealed interface ProxyTrafficMetricsEvent {
    ...
    data class BytesReceived(val bytes: Long) : ProxyTrafficMetricsEvent {
        init {
            require(bytes >= 0) { "Received byte delta must not be negative" }
        }
    }

    data class BytesSent(val bytes: Long) : ProxyTrafficMetricsEvent {
        init {
...
object ProxyTrafficMetricsReducer {
    fun apply(
        ...
        when (event) {
            ProxyTrafficMetricsEvent.ConnectionAccepted -> metrics.copy(
                activeConnections = metrics.activeConnections + 1,
                totalConnections = metrics.totalConnections + 1,
            )
            ProxyTrafficMetricsEvent.ConnectionRejected -> metrics.copy(
            ...
            is ProxyTrafficMetricsEvent.BytesReceived -> metrics.copy(
                bytesReceived = metrics.bytesReceived + event.bytes,
            )
            is ProxyTrafficMetricsEvent.BytesSent -> metrics.copy(
