...
object ManagementApiReadOnlyResponses {
    fun health(status: ProxyServiceStatus): ManagementApiResponse = ManagementApiResponse.json(
        statusCode = 200,
    ...
    fun networks(networks: List<NetworkDescriptor>): ManagementApiResponse = ManagementApiResponse.json(
        statusCode = 200,
        body =
    ...
    fun publicIp(publicIp: String?): ManagementApiResponse = ManagementApiResponse.json(
        statusCode = 200,
...
private fun ProxyServiceStatus.serviceJson(): String = buildString {
    append('{')
    append(""""state":""")
...
private fun NetworkDescriptor.networkJson(): String = buildString {
    append('{')
    append(""""id":""")
...
private fun RouteTarget.apiValue(): String = when (this) {
...
private fun NetworkCategory.apiValue(): String = when (this) {
    NetworkCategory.WiFi -> "wifi"
    NetworkCategory.Cellular -> "cellular"
    NetworkCategory.Vpn -> "vpn"
}

private fun CloudflareTunnelState.apiValue(): String = when (this) {
...
private fun RootAvailabilityStatus.apiValue(): String = when (this) {
...
private fun ProxyServiceStatus.rootAvailabilityFor(rootOperationsEnabled: Boolean): RootAvailabilityStatus = if (rootOperationsEnabled) {
    rootAvailability
} else {
    RootAvailabilityStatus.Unknown
}

private fun ProxyStartupError.apiValue(): String = when (this) {
