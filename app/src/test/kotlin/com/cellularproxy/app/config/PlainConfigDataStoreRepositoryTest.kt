...
class PlainConfigDataStoreRepositoryTest {
    ...
    fun `saves config and replaces stale nullable values`() = withRepository { repository, dataStore ->
        val configWithHostname = AppConfig(
            proxy = ProxyConfig(
        ...
        val configWithoutHostname = AppConfig.default().copy(
        ...
        assertFalse(
            PlainConfigPreferenceKeys.cloudflareManagementHostnameLabel in dataStore.data.first().asMap().keys,
        )
    }
}
