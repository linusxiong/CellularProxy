...
class ServiceRestartRootControllerTest {
    ...
    fun `restart runs service restart command and reports success`() {
        ...
        val controller = ServiceRestartRootController(
            executor = serviceRestartExecutor(calls) {
    ...
    fun `result invariants reject contradictory service restart command details`() {
        ...
        val mobileDataSuccessExecution = serviceRestartExecution(
    ...
    private fun serviceRestartTimeoutExecution(): RootCommandExecution {
        val result = RootCommandResult.timedOut(
        ...
        return RootCommandExecution.completed(
            result = result,
            started = com.cellularproxy.shared.root.RootCommandAuditRecord.started(RootCommandCategory.ServiceRestart),
            completed = com.cellularproxy.shared.root.RootCommandAuditRecord.completed(result),
            rawStdout = "",
        )
    }
